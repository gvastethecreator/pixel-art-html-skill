---
name: pixel-art-html
description: "Pixel-art HTML artifacts: design, repixelize, critique, and package exact grids with editable palette JSON, PNG parity, multi-resolution conversion, and browser proof."
---
...
1. Set the contract.
   ...
   - Default to 32x32, at most 16 colors, transparent background, `contain`, and no dithering only when the request does not decide them.
...
3. Build structure before detail.
...
4. Compile through the public script.
...
6. Handle resolution sets deliberately.
...
Useful image options: `--size N`, `--width N --height N`, `--sizes all`, `--colors 4|8|16|32`, `--fit contain|cover|stretch`, `--dither none|floyd`, `--background transparent|#RRGGBB`, `--alpha-threshold 0..255`, and `--scale N`.
