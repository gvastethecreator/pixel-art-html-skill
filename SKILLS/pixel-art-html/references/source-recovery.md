...
Use this if you convert an uncertain image source to an exact 8x8-32x32 runtime grid. Recovery improves cell placement; it does not replace low-resolution art direction or manual pixel editing.

## 1. Classify before packing

The local target-aware classifier emits one of three source classes:

- `exact-grid`: hard, internally uniform cells align to a plausible lattice. Nearest-neighbor preservation is safe only if that lattice exactly matches the requested target.
...
Canonical `source.analysis` records confidence, detector, target grid, inferred grid if available, metrics, and human-readable reasons. A reviewer can use `--source-class` to override the class, but the override is recorded as `forced` / `user-override`. Detection never changes `--size`, `--sizes`, `--width`, or `--height`.

## 2. Choose the reconstruction route

`--reconstruction auto` is the normal path:

1. Preserve an exact-grid source with nearest sampling if its inferred lattice matches the target.
...
This split stops anti-aliased boundary colors from voting as subject structure. `--structure-colors 0` derives a temporary budget from the requested final palette. Explicit values from 2 through 64 are available for controlled experiments.

`--reconstruction legacy` retains resize-first conversion for regression comparison. `--reconstruction two-stage` forces target-aware packing even if the source appears exact.
...
The overlay is diagnostic. It adaptively skips source lines if they are closer than four display pixels. Browser metrics record the displayed stride. A high-confidence lattice can still yield a poor 8x8 icon. Recognition, pose, material, and focal hierarchy are not detector outputs.
...
- use at most one or two single-cell accents, only if they survive native 1x review
- compare without the title or source image visible

Image-derived canonical metadata sets `manual_repair_required` for these sizes unless an exact matching lattice was preserved. The proof page repeats that as a review warning so a clean validator result cannot hide the missing authoring pass.
...
The compiler rejects missing decisions, dimension drift, and zero-cell no-ops. Canonical output preserves the full baseline grid/palette, repair decisions, original image analysis if present, and recomputed changed/painted-cell evidence. Its proof page shows baseline and repair at equal scale. Blind mode hides the artifact title and changes captions to neutral `Sample A` / `Sample B` labels. These fields prove a reviewable authored intervention, never automatic artistic acceptance.
...
The benchmark authors known masters at 8, 16, 24, and 32 cells. It applies deterministic nearest, fractional, soft, affine-drift, noise, and JPEG degradations. It compares `legacy` with `two-stage`. It writes:
...
- runs once and reuses the result across a resolution set
